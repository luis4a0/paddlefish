...
unsigned Document::add_truetype_font(TrueTypeFontPtr font, bool embed)
{
...
PADDLEFISH_ONLY_TRUETYPE(
  ...
  unsigned cid_to_gid_map_id = add_custom_stream(cid_to_gid::create_map(font));

  // Set the reference to the CID to GID map.
  f->set_map_ref(cid_to_gid_map_id);
  ...
  return std::make_pair(first_body_object_number + total_body_objects - 1,
                        f->get_name());
)
}
